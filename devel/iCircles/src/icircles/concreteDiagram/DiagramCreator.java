...
/**
 * An interface to provide a rough idea of sizes for zones
 * (AbstractBasicRegions) and contours (AbstractCurves), beginning with an
 * AbstractDescription and some recomposition steps (these determine the order
 * in which we will (re)construct the concrete diagram). The get methods should
 * return big numbers for zones and contours which ought to be drawn large to
 * accommodate other diagram features.
 * 
 */
...
class JeansGuideSizeStrategy implements GuideSizeStrategy {
	...
	public JeansGuideSizeStrategy(AbstractDescription ad,
		...
		while (cIt.hasNext()) {
			...
			// zone scores.
			double guide_size = Math.exp(0.75 * Math.log(cScore
					/ totalZoneScore)) * 200;
	...
	/**
	 * Apply a heuristic to gauge how big a zone should be in a drawn diagram.
	 * For the moment, a simple heuristic says that each zone contributes the
	 * same value to the heuristics. In the future, we might decide to weight
	 * some zones so that they provide larger values to the heuristics.
	 * 
	 * @param abr
	 * @return
	 */
	private static double scoreZone(AbstractBasicRegion abr,
...
/**
 * Determine the order in which we build a diagram (i.e. choose in which order
 * circles will be placed). Optimisations include choosing a good order in which
 * to add circles or choosing to draw lots of circles at once. The BuildStep is
...
/**
 * The simplest BuildStepMaker takes a sequence of recomposition steps and
 * re-expresses those as a sequence of BUildSteops which each adds one contour.
 */
...
/**
 * A better BuildStepMaker re-orders and combines the steps which add contours
 * so that we can add groups of similar contours together to be more able to
 * build diagrams with symmetry.
 */
...
class JeansBuildStepMaker implements BuildStepMaker {
	...
	public BuildStep make() {
		...
		SimpleBuildStepMaker simpleConverter = new SimpleBuildStepMaker(
				recompSteps);
		...
		while (bs != null) {
			...
			// circle).
			DEB.assertCondition(bs.recomp_data.size() == 1,
					"not ready for multistep");
			//
			if (bs.recomp_data.get(0).split_zones.size() == 1) {
				...
				while (beforefuturebs != null && beforefuturebs.next != null) {
					...
					if (rd2.split_zones.size() == 1) {
						...
						if (abr.isLabelEquivalent(abr2)) {
							DEB.out(2, "found matching abrs " + abr.debug()
									+ ", " + abr2.debug());
							// check scores match
							double abrScore = guideSizes
									.getGuideSize(rd.added_curve);
							double abrScore2 = guideSizes
									.getGuideSize(rd2.added_curve);
							DEB.assertCondition(abrScore > 0 && abrScore2 > 0,
									"zones must have score");
							DEB.out(2, "matched nestings " + abr.debug()
									+ " and " + abr2.debug()
									+ "\n with scores " + abrScore + " and "
									+ abrScore2);
				...
				while (beforefuturebs != null && beforefuturebs.next != null) {
					...
					if (rd2.split_zones.size() == 2) {
						...
						if ((abr1.isLabelEquivalent(abr3) && abr2
								.isLabelEquivalent(abr4))
								|| (abr1.isLabelEquivalent(abr4) && abr2
										.isLabelEquivalent(abr3))) {

							DEB.out(2, "found matching abrs " + abr1.debug()
									+ ", " + abr2.debug());
							// check scores match
							double abrScore = guideSizes
									.getGuideSize(rd.added_curve);
							...
							DEB.assertCondition(abrScore > 0 && abrScore2 > 0,
									"zones must have score");
							DEB.out(2, "matched piercings " + abr1.debug()
									+ " and " + abr2.debug()
									+ "\n with scores " + abrScore + " and "
									+ abrScore2);
							if (abrScore == abrScore2) {
...
/**
 * The primary task of a DiagramCreator is to take an AbstractDescription and
 * produce a ConcreteDiagram. The constructors take in the AbstractDescription
 * and potentially some more information to influence some of the choices which
 * are made (the heuristics) when we define the geometry of the ConcreteDiagram.
 * The main work takes place in DiagramCreator::createDiagram. The diagrams are
 * drawn iteratively - we successively add groups of contours. We can this of
 * the diagram as being built up in stages. In the simplest case we add only one
 * new contour at each stage, but to assist with symmetry, we sometimes add
 * multiple contours in one stage. Not all diagrams can be drawn, because we try
 * to ensure that contours can be visually distinguished and we might not find
 * enough space to place them far enough apart. Except in very extreme
 * circumstances, this could be considered a bug - a different choice at an
 * earlier stage might have made it easier to fit a later contour into the
 * diagram. The task of taking an AbstractDescription and splitting the creation
 * task into stages is called Decomposition/Recomposition. The heuristics used
 * to make choices there can be controlled using Strategy objects, passed to the
...
public class DiagramCreator {
	...
	/**
	 * In this constructor, we take the abstract description, and do some
	 * analysis of it. We build a decomposition and recomposition, using the
	 * default strategies.
	 */
	...
	public ConcreteDiagram createDiagram(int size) throws CannotDrawException {
		...
		try {
			...
			CircleContour.fitCirclesToSize(drawnCircles, size); // scales to
																// requested
																// size
			...
			// ConcreteDiagram.
			result = new ConcreteDiagram(new Rectangle2D.Double(0, 0, size,
					size), drawnCircles, shadedZones, unshadedZones, spiders);
			result.setFont(font);
			DEB.showFilmStrip();
	...
	private boolean foot_is_on_leg(ConcreteSpiderFoot foot,
			ConcreteSpiderLeg leg, double tol) {
		...
		double sf_perp_leg_len = Math.sqrt(sf_perp_leg_x * sf_perp_leg_x
				+ sf_perp_leg_y * sf_perp_leg_y);
		...
		if (DEB.level >= 3) {
			System.out.println("sf_perp_leg_len = " + sf_perp_leg_len
					+ ", sf_prop_leg = " + sf_prop_leg);
		}

		boolean foot_on_leg = sf_perp_leg_len < tol && sf_prop_leg > 0
				&& sf_prop_leg < 1;
		return foot_on_leg;
	}
	...
	private ArrayList<ConcreteSpider> createSpiders()
		...
		HashMap<AbstractBasicRegion, ArrayList<ConcreteSpiderFoot>> drawnFeet = new HashMap<AbstractBasicRegion, ArrayList<ConcreteSpiderFoot>>();
		for (AbstractBasicRegion abr : footCount.keySet()) {
			...
			AbstractBasicRegion zone_in_last_diag = last_diag
					.getLabelEquivalentZone(abr);
		...
		while (it.hasNext()) {
			...
			for (ConcreteSpiderFoot centreCandidate : cs.feet) {
				...
				for (ConcreteSpiderFoot other : cs.feet) {
					...
					distSum += Math
							.sqrt((centreCandidate.getX() - other.getX())
									* (centreCandidate.getX() - other.getX())
									+ (centreCandidate.getY() - other.getY())
									* (centreCandidate.getY() - other.getY()));
				}
					...
					for (ConcreteSpider cs2 : spiders) {
						...
						for (ConcreteSpiderLeg leg : cs2.legs) {
							if (leg.from == foot || leg.to == foot) {
								// this leg is bound to be close to foot -it's
								// attached!
								continue;
							}
							// is foot on leg?
							if (DEB.level >= 3) {
								System.out.println("check spider " + cs
										+ " foot (" + foot.getX() + ","
										+ foot.getY() + ") against "
										+ " spider " + cs2 + " leg ("
										+ leg.from.getX() + ","
										+ leg.from.getY() + ")->("
										+ leg.to.getX() + "," + leg.to.getY()
										+ ")");
							}
							boolean foot_on_leg = foot_is_on_leg(foot, leg, tol);
							if (foot_on_leg) {
								// nudge the foot, but check it's still in its
								// zone afterwards
								double old_x = foot.getX();
								double old_y = foot.getY();
								AbstractBasicRegion abr = feet_and_zones
										.get(foot);

								ConcreteZone cz = makeConcreteZone(abr);
								Area a = new Area(cz.getShape(box));

								double new_y = old_y + 5 * tol; // how far to
																// nudge it?
								// double new_x = old_x + 2 * tol;
								double new_x = old_x;
								CircleContour test = new CircleContour(new_x,
										new_y, tol, null);
								if (circleInArea(test, a)) {
									...
									if (foot_is_on_leg(foot, leg, tol)) {
										...
										check_feet_placements = true; // if we
																		// moved
																		// one,
																		// start
																		// all
								...
								new_x = old_x - 5 * tol;
								new_y = old_y - 5 * tol;
								test = new CircleContour(new_x, new_y, tol,
										null);
								if (circleInArea(test, a)) {
									foot.setX(new_x);
									foot.setY(new_y);
								...
								if (circleInArea(test, a)) {
									...
									if (foot_is_on_leg(foot, leg, tol)) {
										...
										// if we moved one, start all over again!
										check_feet_placements = true; 
										break;
									}
								}
								new_x = old_x - 5 * tol;
	...
	/**
	 * Apply heuristics to abstract diagram contours to decide how large they
	 * might be (a "guide size" for the contour). Populates the map guideSizes.
	 */
	...
	/**
	 * Given drawnCircles create the ConcreteZones we will need in the drawn
	 * ConcreteDiagram. Call this only after createCircles has successfully done
	 * its job.
	 * 
	 * @param shadedZones
	 * @param unshadedZones
	...
	private void createZones(ArrayList<ConcreteZone> shadedZones,
		...
		// which zones in final_diagram were not in initial_diagram, or
		// specified shaded in initial_diagram?

		if (DEB.level > 2) {
			Iterator<AbstractBasicRegion> it = abstractDiagram
					.getZoneIterator();
			while (it.hasNext()) {
		...
		while (it.hasNext()) {
			...
			AbstractBasicRegion matched_z = abstractDiagram
					.getLabelEquivalentZone(z);
	...
	/**
	 * Given the abstract diagram description, the decomposition / recomposition
	 * and some guide sizes for contours based on heuristics, make choices of
	 * drawn circles for the contours in the abstract diagram.
	 * 
	...
	private boolean createCircles() throws CannotDrawException {
		...
		stepLoop: while (thisBuildStep != null) {
			...
			if (thisBuildStep.recomp_data.size() > 1) {
				...
				if (thisBuildStep.getType() == BuildStep.Piercing.NESTED_PIERCING) {
					...
					if (cs != null && cs.size() > 0) {
						DEB.assertCondition(
								cs.size() == thisBuildStep.recomp_data.size(),
								"not enough circles for rds");
						for (int i = 0; i < cs.size(); i++) {
							CircleContour c = cs.get(i);
							ac = thisBuildStep.recomp_data.get(i).added_curve;
							DEB.assertCondition(
									c.ac.getLabel() == ac.getLabel(),
									"mismatched labels");
					...
					CircleContour pierced_cc = abstractToConcreteContourMap
							.get(pierced_ac);
					...
					double suggested_rad = guideSizes
							.getGuideSize(piercingCurve);
					...
					for (AbstractBasicRegion abr : allZones) {
						DEB.out(1, "compare " + abr.debug() + " against "
								+ piercingCurve.debug());
						if (!abr.is_in(piercingCurve)) {
							continue;
						}
						DEB.out(1, "OK " + abr.debug() + " is in "
								+ piercingCurve.debug()
								+ ", so compare against " + pierced_ac.debug());
					...
					DEB.out(3, "scores for " + piercingCurve + " are inside="
							+ score_in_c + " and outside=" + score_out_of_c);
					...
					if (score_out_of_c > score_in_c) {
						...
						double nudge = Math.min(suggested_rad * 0.3,
								(pierced_cc.radius * 2 - suggested_rad) * 0.5);
						center_of_circle_lies_on_rad -= nudge;
					}

					double guide_rad = guideSizes
							.getGuideSize(thisBuildStep.recomp_data.get(0).added_curve);
					int sampleSize = (int) (Math.PI / Math.asin(guide_rad
							/ pierced_cc.radius));
					if (sampleSize >= thisBuildStep.recomp_data.size()) {
						int num_ok = 0;
						for (int i = 0; i < sampleSize; i++) {
							double angle = i * Math.PI * 2.0 / sampleSize;
							double x = pierced_cc.cx + Math.cos(angle)
									* center_of_circle_lies_on_rad;
							double y = pierced_cc.cy + Math.sin(angle)
									* center_of_circle_lies_on_rad;
							if (a.contains(x, y)) {
								CircleContour sample = new CircleContour(
										x,
										y,
										guide_rad,
						...
						if (num_ok >= thisBuildStep.recomp_data.size()) {
							if (num_ok == sampleSize) {
								// all OK.
								for (int i = 0; i < thisBuildStep.recomp_data
										.size(); i++) {
									double angle = 0.0 + i * Math.PI * 2.0
											/ thisBuildStep.recomp_data.size();
									double x = pierced_cc.cx + Math.cos(angle)
											* center_of_circle_lies_on_rad;
									double y = pierced_cc.cy + Math.sin(angle)
											* center_of_circle_lies_on_rad;
									if (a.contains(x, y)) {
										AbstractCurve added_curve = thisBuildStep.recomp_data
												.get(i).added_curve;
										CircleContour c = new CircleContour(x,
												y, guide_rad, added_curve);
										abr0 = thisBuildStep.recomp_data.get(i).split_zones
												.get(0);
										abr1 = thisBuildStep.recomp_data.get(i).split_zones
												.get(1);

										abstractToConcreteContourMap.put(
												added_curve, c);
										addCircle(c);
									}
								}
								thisBuildStep = thisBuildStep.next;
								continue stepLoop;
							} else if (num_ok > sampleSize) { // BUG? Doesn't
																// make sense
								num_ok = 0;
								for (int i = 0; i < sampleSize; i++) {
									double angle = 0.0 + i * Math.PI * 2.0
									...
									if (a.contains(x, y)) {
										...
										CircleContour c = new CircleContour(x,
												y, guide_rad, added_curve);
										if (circleInArea(c, a)) {
											abr0 = thisBuildStep.recomp_data
													.get(num_ok).split_zones
													.get(0);
											abr1 = thisBuildStep.recomp_data
													.get(num_ok).split_zones
													.get(1);
											abstractToConcreteContourMap.put(
													added_curve, c);
											addCircle(c);
											num_ok++;
											if (num_ok == thisBuildStep.recomp_data
													.size()) {
												break;
											}
										}
...
//		DEB.show(4, a, "area for " + debugIndex);
		...
		if (zone.getNumContours() > 0 && abstractCurves.size() == 1) {
			...
			DEB.assertCondition(zoneInLast != null,
					"failed to locate zone in final diagram");
			...
			if (nbring_curves.size() == 1) {
				...
				DEB.assertCondition(acOutside != null,
						"did not find containing contour");
				CircleContour ccOutside = abstractToConcreteContourMap
						.get(acOutside);
				DEB.assertCondition(ccOutside != null,
						"did not find containing circle");
				if (ccOutside != null) {
					DEB.out(2, "putting contour " + abstractCurves.get(0)
							+ " inside " + acOutside.getLabel());
					double rad = Math.min(guideRadius, ccOutside.radius
							- smallestRadius);
					if (rad > 0.99 * smallestRadius) {
						// build a co-centric contour
						CircleContour attempt = new CircleContour(ccOutside.cx,
								ccOutside.cy, rad, abstractCurves.get(0));
						if (circleInArea(attempt, a)) {
							if (rad > 2 * smallestRadius) // shrink the
															// co-centric
															// contour a bit
							{
								attempt = new CircleContour(ccOutside.cx,
										ccOutside.cy, rad - smallestRadius,
										abstractCurves.get(0));
							}
							result.add(attempt);
							return result;
						}
					...
					System.out
							.println("warning : did not find expected containing circle...");
				}
			} else if (nbring_curves.size() == 2) {
				// we should put a circle along the line between two existing
				// centres
				...
				if (cc1 != null && cc2 != null) {
					...
					double step_c1_c2_len = Math.sqrt(step_c1_c2_x
							* step_c1_c2_x + step_c1_c2_y * step_c1_c2_y);
					...
					double p1x = cc1.cx + unit_c1_c2_x * cc1.radius
							* (in2 ? 1.0 : -1.0);
					double p2x = cc2.cx + unit_c1_c2_x * cc2.radius
							* (in1 ? -1.0 : +1.0);
					double cx = (p1x + p2x) * 0.5;
					double max_radx = (p2x - p1x) * 0.5;
					double p1y = cc1.cy + unit_c1_c2_y * cc1.radius
							* (in2 ? 1.0 : -1.0);
					double p2y = cc2.cy + unit_c1_c2_y * cc2.radius
							* (in1 ? -1.0 : +1.0);
					double cy = (p1y + p2y) * 0.5;
					double max_rady = (p2y - p1y) * 0.5;
					double max_rad = Math.sqrt(max_radx * max_radx + max_rady
							* max_rady);

					// build a contour
					CircleContour attempt = new CircleContour(cx, cy, max_rad
							- smallestRadius, abstractCurves.get(0));
					// DEB.show(3, attempt.getBigInterior());
					if (circleInArea(attempt, a)) {
						if (max_rad > 3 * smallestRadius) // shrink the
															// co-centric
															// contour a bit
						{
							attempt = new CircleContour(cx, cy, max_rad - 2
									* smallestRadius, abstractCurves.get(0));
						} else if (max_rad > 2 * smallestRadius) // shrink the
																	// co-centric
																	// contour a
																	// bit
						{
							attempt = new CircleContour(cx, cy, max_rad
									- smallestRadius, abstractCurves.get(0));
						}
						result.add(attempt);
						return result;
		...
		if (abstractCurves.get(0) == null) {
			...
			DEB.out(2, "putting contour " + abstractCurves.get(0).getLabel()
					+ " inside a zone - grid-style");
		}
		...
		for (int i = 0; i < ni; i++) {
			...
			for (int j = 0; j < nj; j++) {
				...
				contained[i][j] = new PotentialCentre(cx, cy,
						a.contains(cx, cy));
				if (DEB.level > 3) {
					if (contained[i][j].ok) {
						System.out.print("o");
		...
		for (int i = 0; i < ni; i++) {
			for (int j = 0; j < nj; j++) {
				...
				for (int sq = size + 1; sq < max_sq + 1; sq++) {
					...
					DEB.out(2, "look for a box from (" + i + "," + j
							+ ") size " + sq);

					if (all_ok_in(i, i + (sq * abstractCurves.size()) + 1, j, j
							+ sq + 1, contained, ni, nj)) {
						DEB.out(2, "found a wide box, corner at (" + i + ","
								+ j + "), size " + sq);
						corneri = i;
						cornerj = j;
						size = sq;
						isTall = false;
							...
							&& all_ok_in(i, i + sq + 1, j, j
									+ (sq * abstractCurves.size()) + 1,
									contained, ni, nj)) {
						DEB.out(2, "found a tall box, corner at (" + i + ","
								+ j + "), size " + sq);
						corneri = i;
						cornerj = j;
						size = sq;
						...
						break; // neither wide nor tall worked - move onto next
								// (x, y)
					}
		...
		if (size > 0) {
			...
			for (int labelIndex = 0; centredCircles != null
					&& labelIndex < abstractCurves.size(); labelIndex++) {
				...
				CircleContour attempt = new CircleContour(x, y, Math.min(
						guideRadius, actualRad), ac);
				// DEB.show(3, attempt.getBigInterior());
				if (circleInArea(attempt, a)) {
			...
			for (int labelIndex = 0; labelIndex < abstractCurves.size(); labelIndex++) {
				...
				CircleContour attempt = new CircleContour(x, y, Math.min(
						guideRadius, actualRad + smallestRadius), ac);
				if (circleInArea(attempt, a)) {
					result.add(attempt);
				} else {
			...
			throw new CannotDrawException(
					"cannot fit nested contour into region");
		}
	}
	 ...
	 * @param size
	 */
	private void addDebugView(int deb_level, // only show if deb_level >= global
												// debug level
			int debug_frame_index, int size) {
		...
		ConcreteDiagram cd = new ConcreteDiagram(new Rectangle2D.Double(0, 0,
				size, size), circles_copy, shadedZones, unshadedZones, spiders);
		CirclesPanel cp = new CirclesPanel("debug frame " + debug_frame_index,
				"no failure", cd, true);
...
/**
 * When considering a set of possible positions for a circle's centre, we can
 * build PotentialCentre objects to log considered positions and whether they
 * were OK (relative to other circles) or not.
 * 
 * @author
 * 
 */
